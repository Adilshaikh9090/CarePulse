  ...
  return (
    <>
      <button
        ...
        className="fixed bottom-24 right-5 z-40 flex h-13 w-13 lg:bottom-5 items-center justify-center rounded-full bg-sky-600 p-3.5 text-white shadow-lg shadow-sky-500/30 transition hover:bg-sky-700"
      >
      ...
      {open && (
        <div className="fixed bottom-20 right-5 z-40 flex h-[440px] w-[min(92vw,360px)] glass flex-col overflow-hidden rounded-2xl shadow-2xl ring-1 ring-linestrong">
              ...
              <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <p className={`max-w-[82%] rounded-xl px-3 py-2 text-xs leading-relaxed ${
                  m.role === 'user'
                    ...
                    : 'bg-subtle text-slate-300 ring-1 ring-line'
                }`}>
                  {m.content}
                </p>
              </div>
              ...
              <div className="flex flex-wrap gap-1.5 pt-1">
                {QUICK_PROMPTS.map((q) => (
                  <button key={q} onClick={() => send(q)}
                          className="rounded-full bg-subtle px-2.5 py-1 text-[10px] text-slate-400 ring-1 ring-linestrong hover:text-slate-200">
                    {q}
                  </button>
                ))}
              </div>
          ...
          <footer className="flex items-center gap-2 border-t border-line p-2.5">
            <input
              value={input}
            ...
            <button onClick={() => send()} disabled={busy || !input.trim()}
                    className="rounded-lg bg-sky-600 p-2 text-white disabled:opacity-40">
              <Send size={14} />
            </button>
          </footer>
        </div>

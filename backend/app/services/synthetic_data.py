...
def build_history(profile: dict, user_id: int, today: date, days: int, is_demo: bool = False):
    ...
    rows = {}
    for t in range(days):
        day = today - timedelta(days=days - 1 - t)
...
def checkin_payload(row: dict, prev_workload: int | None) -> dict:
    ...
    slope = (row["workload"] - prev_workload) if prev_workload is not None else 0
    return {
        "workload_score": row["workload"] * 2,
        "fatigue_score": row["fatigue"] * 2,
        "sleep_quality": float(row["sleep_quality"]),
        "duty_hours": float(row["duty_hours"]),
        "overtime_frequency": 10.0 if row["overtime"] else 3.0,
        "job_satisfaction": row["job_satisfaction"] * 2,
        "rest_break_quality": BREAK_MAP.get(row["rest_breaks"], 3.0),
        "self_reported_stress": (6 - row["feeling"]) * 2,
